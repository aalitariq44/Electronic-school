...
class PostsPage extends StatefulWidget {
  ...
  const PostsPage({super.key, required this.type});
...
class _PostsPageState extends State<PostsPage> {
  ...
  String _currentPostType = 'teacher'; // القيمة الافتراضية هي 'teacher'
  String appbarTitle = 'منشورات المدرسين'; // القيمة الافتراضية هي 'teacher'
  ...
  void _initPostsStream() {
    _postsStream = FirebaseFirestore.instance
        .collection('posts')
        .where('type', isEqualTo: _currentPostType)
  ...
  void _updatePostType(String newType) {
    setState(() {
      _currentPostType = newType;
      _postsLimit = 4; // إعادة تعيين حد المنشورات عند تغيير النوع
    });
    _initPostsStream();
  }
      ...
      appBar: AppBar(
        ...
        actions: [
          ToggleButtons(
            ...
            isSelected: [
              _currentPostType == 'teacher',
              _currentPostType == 'student'
            ],
            onPressed: (index) {
              setState(() {
                if (index == 0) {
                  _updatePostType('teacher');
                  appbarTitle = 'منشورات المدرسين';
                } else {
                  _updatePostType('student');
                  appbarTitle = 'منشورات الطلاب';
                }
              });
            },
            color: Colors.white,
